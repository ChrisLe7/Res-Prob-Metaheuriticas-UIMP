...
public class ProblemMKP extends Problem{
     ...
     *
     * @param individual The individual solution for which the exceeded capacity is to be calculated.
     * @return The total exceeded capacity as a double value. If no capacities are exceeded, returns 0.0.
     */
    private double getExceededCapacityViolation(Individual individual){
        double exceeded_capacity = 0.0;
        double [] sum_weights = new double[nDims_];

        for (int j = 0; j < CL; j++){
            if (individual.get_allele(j) == 1){
                for (int i = 0; i < nDims_; i++){
    ...
    private double calculateProfits(Individual individual){
        double profit = 0.0;
        for (int i = 0; i < CL; i++){
    ...
    private double MDKP(Individual individual) {
        double fitness = getExceededCapacityViolation(individual) * -1.0;
        if (fitness == 0){
            fitness = calculateProfits(individual);
        }
        return fitness;
    }
}
